...
import { ArrowRight, Cpu, Layers, Server, Smartphone, Globe } from 'lucide-react';
...
const Services: React.FC = () => {
  ...
  return (
    <section ref={containerRef} id="services" className="py-32 bg-background relative border-t border-border transition-colors duration-300">
        ...
        <div className="services-header grid grid-cols-1 lg:grid-cols-12 gap-12 mb-20">
           ...
           <div className="lg:col-span-6 flex justify-start lg:justify-end items-end">
              <p className="text-muted-foreground text-lg max-w-md lg:text-right">
                 We engineer intelligent systems designed for automation, scalability, and long-term efficiency.
              </p>
           </div>
        </div>
              ...
              <div className="sticky top-32 w-full aspect-video lg:aspect-[4/5] border border-border bg-foreground/5 p-2">
                 <div className="relative w-full h-full overflow-hidden bg-black">
                    ...
                    <div className="absolute bottom-8 left-8 z-20">
                       <div className="text-white text-xs font-mono uppercase tracking-widest mb-1">System Status</div>
                       <div className="flex items-center gap-2">
                          <div className="w-2 h-2 bg-green-500 rounded-none animate-pulse"></div>
                          <span className="text-white font-bold tracking-widest">ONLINE</span>
                       </div>
                    </div>
              ...
              {services.map((service) => (
                 ...
                 >
                    <div className="flex items-center gap-4 mb-6">
                          <span className="text-sm font-bold tracking-widest text-primary border border-primary/20 px-2 py-1">{service.id}</span>
                          <span className="text-muted-foreground group-hover:text-foreground transition-colors">{service.icon}</span>
                    </div>
